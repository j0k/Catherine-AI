...
class VACore(JaaCore):
    ...
    def setup_assistant_voice(self):
        ...
        try:
            ...
            self.print_error("Error initializing WAV playback plugin (playWavEngineId)", e)
            self.print_red('Try to set in options/core.json: "playWavEngineId": "sounddevice"')
            self.print_red('...temporarily switching to console response output...')
        ...
        try:
            ...
            self.print_error("Error initializing TTS plugin (ttsEngineId)", e)
            cprint('Try to set in options/core.json: "ttsEngineId": "console" to test console output', "red")
            cprint('Later, if everything works, you can configure your TTS engine', "red")

            from sys import platform
            if platform == "linux" or platform == "linux2":
                cprint("More about installing on Linux see Irina docs: https://github.com/janvarev/Irene-Voice-Assistant/blob/master/docs/INSTALL_LINUX.md", "red")
            elif platform == "darwin":
                cprint("More about installing on Mac see Irina docs: https://github.com/janvarev/Irene-Voice-Assistant/blob/master/docs/INSTALL_MAC.md", "red")
            elif platform == "win32":
                pass
        ...
        if self.ttsEngineId2 != self.ttsEngineId:
            try:
                ...
                self.print_error("Error initializing TTS2 plugin (ttsEngineId2)", e)
        ...
        for k in self.fuzzy_processors.keys():
            try:
                ...
                self.print_error("Error initializing fuzzy_processor {0}".format(k), e)
    ...
    def play_voice_assistant_speech(self,text_to_speech:str):
        ...
        if not is_processed:
            print("Error while outputting TTS - remoteTTS was not processed.")
            print("Current remoteTTS: {}".format(self.remoteTTS))
            print("Current remoteTTSList: {}".format(remoteTTSList))
            print("Expected remoteTTS (for example): 'none'")
